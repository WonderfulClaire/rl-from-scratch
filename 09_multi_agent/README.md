...
环境：5×5 网格，2 个智能体从对角出发，目标是**同时**到达中心区域（都在中心才有团队奖励 +10，每步 -0.1；只有一个到达没有奖励——必须协调）。

对比：IPPO（各自局部 Critic）vs MAPPO（中心化 Critic 看到两个人的位置）。预期 MAPPO 收敛更快更稳——"我该不该等他"这类协调信息只有中心化 Critic 能编码。
