...
This repository ties three things together: derivations, learning implementations, and experiment records; validation scope should be checked for each chapter.
...
The repository includes standard and custom environments; saved benchmark files contain aggregate curves rather than complete per-seed trajectories. Learning curves come from **real runs with fixed seeds**. **Failure cases are reported honestly too** — e.g. in Chapter 11 the DQN trader underperforms buy-and-hold on the test split, a live demonstration of the backtest-overfitting trap.
...
> Verification note: saved curve files contain cross-seed aggregates. Algorithms have different final step counts, and training termination is not a measured first-solve time. See the [data and metric audit](results/RESULTS.md).

## 📊 Results (real runs)

> The saved plots are documented as **mean ± std over 3 random seeds**. CartPole and Pendulum use environment steps; the multi-agent experiment uses training iterations. Available data and verification limits are recorded in [results/RESULTS.md](results/RESULTS.md).

### CartPole-v1: Four Mainstream Algorithms Head-to-Head

![CartPole learning curves](results/cartpole_curves.png)

*Saved training curves for four implementations. Their endpoints have different step counts and cannot be directly ranked as a common-budget evaluation.*

### Pendulum-v1: Three Continuous-Control Algorithms

![Pendulum learning curves](results/pendulum_curves.png)

*Saved curves for three continuous-control implementations; any comparison is specific to this task and configuration.*
